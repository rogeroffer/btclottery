...
// #RF best share reset after a new block was found
static bool opt_bestshare_r;
...
// #RF bestshare reset after new block was found
	OPT_WITHOUT_ARG("--bestsharereset",
			opt_set_bool, &opt_bestshare_r,
			"Resets the best share after a new block was found"),			
...
static void curses_print_status(void)
{
	...
	wclrtoeol(statuswin);
// #RF Fixed character +1 space at the end to get rid of the 'K' / 'M' / 'T' fragment after zero_stats
...
// #RF add new best share report to notice area
	if(new_best == true)
		applog(LOG_NOTICE, "New best share reached: %s", best_share);
...
// #RF Reset best share if a new block was found
	if (opt_bestshare_r == true)	
	best_diff = 0;
...
// #RF add new function best share reset after a new block was found
		  "[B]est share resets after a new block is found:%s\n"		  
		...
		// #RF best share reset after new block was found 
		opt_bestshare_r ? "enabled" : "disabled",		
...
// #RF best share reset after a new block was found	
		...
		opt_bestshare_r ^= true;
		wlogprint("best share reset %s\n", opt_compact ? "enabled" : "disabled");
		switch_logsize(false);
		goto retry;		
...
# ifdef __linux
	...
	// #RF switch cursor OFF against flickering arround	
		curs_set(0);
#endif
